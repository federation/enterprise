...
VALUES ($1, $2, $3, $4, $5)',
  ...
  getByName(name: string): Promise<pg.QueryResult> {
    return connection().query(
...
WHERE name = $1',
  ...
  getByRefreshToken(id: string, refreshToken: string): Promise<pg.QueryResult> {
    return connection().query(
...
WHERE account_id = $1 AND refresh_token = $2',
  ...
  updateRefreshToken(id: string, refreshToken: string): Promise<pg.QueryResult> {
    return connection().query(
